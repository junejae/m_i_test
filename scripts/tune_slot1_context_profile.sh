...
if (( TARGET_LEN >= 8192 )); then
  ...
  set_kv "VLLM_EXTRA_ARGS_1" "--swap-space 16 --enforce-eager"
elif (( TARGET_LEN >= 4096 )); then
  set_kv "GPU_MEMORY_UTILIZATION_1" "0.92"
  set_kv "VLLM_EXTRA_ARGS_1" "--swap-space 8 --enforce-eager"
else
...
echo "Verify context:"
echo "  curl -sS http://127.0.0.1:\${PORT_1:-8101}/v1/chat/completions -H 'Content-Type: application/json' -d '{\"model\":\"gemma3-4b-it\",\"messages\":[{\"role\":\"user\",\"content\":\"긴 문맥 테스트\"}],\"max_tokens\":64}'"
