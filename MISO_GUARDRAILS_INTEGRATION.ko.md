...
- Golden Set은 **금지 목록**이 아니라, 해당 정책에서 기대하는 **정상 업무 범위의 기준 예시 집합**입니다.
- 프롬프트 인젝션 패턴이나 blocklist가 “이건 막자”라면, Golden Set은 “이 서비스가 원래 처리하려는 질문/요청은 이런 것들이다”를 정의합니다.
- relevance analyzer가 켜져 있을 때, 입력 텍스트를 Golden Set 예시들과 비교해 현재 요청이 정상 업무 범위에 얼마나 가까운지 점수화하는 데 사용됩니다.

대표 용도:

1. 정상 업무 범위와 무관한 요청 감지
2. 우회성 프롬프트 또는 오프토픽 요청의 보조 판별
3. 고객사/서비스별 업무 범위를 정책 단위로 차별화

예시:

- 고객센터 정책이라면 Golden Set에 아래 같은 예시를 넣을 수 있습니다.
  - `계정 비밀번호 초기화 절차를 알려줘`
  - `로그인 오류 해결 방법을 안내해줘`
  - `사내 헬프데스크 접수 절차를 설명해줘`
- 이렇게 두면 입력이 이런 예시들과 너무 멀리 떨어져 있는지 relevance 관점에서 판단할 수 있습니다.

정리:

- `Prompt Injection Patterns`: 공격 패턴 기반 차단
- `Blocklist`: 명시 문구 기반 차단
- `Golden Set`: 정상 업무 범위 기준 예시 데이터

현재 목업 서버 기준 주의:

- Golden Set API와 저장 구조는 이미 존재합니다.
- 다만 현재는 relevance를 기본적으로 **OFF**로 두고 있고, 운영상 강한 차단(enforce) 판단에는 아직 사용하지 않고 있습니다.
- 즉 지금 시점의 Golden Set은 “향후 relevance 기반 정책 확장을 위한 기준 데이터” 성격이 더 큽니다.
